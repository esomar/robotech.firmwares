...
begin
//запоминаем состояние таймера ручного управления,отключаем его.
...
var i:integer;
begin
//проверяем, не кончился ли файл
if not eof(input) then
begin
//читаем 5 команд из файла, выводим их в терминал
  for i:=1 to 5 do
    begin
    readln(s);
    if ComPort.Write(s+#10+#13)=0 then Form1.Caption:='error';
    if ComPort.Read(s)>0 then
      begin // если что-то прочиталось
      Memo1.Lines.Add(s);
      end;
  end;
end
 ...
 begin
 //выключаем таймер воспроизведения
 Timer2.Enabled:=False;
 //восстанавливаем таймер ручного управления
